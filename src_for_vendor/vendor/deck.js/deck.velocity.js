/**
 * deck.velocity.js
 * (c) Stephen A. Butterfill 2014 - 2017
 * 
 * depends on deck.slideActions (included below, last)
 * 
...
(function($, deck, undefined) {
  ...
  var doInit = function doInit() {
    ...
    $.each(slides, function(i, $slide) {
      var actions = getActions($slide);
      $.each( actions, function(_ignore, action){
        $[deck]('addSlideAction', i, action);
      });
    });
    
    // forceRefresh();
    
    $(document).trigger('deckVelocityInit')
    
  };
  
  $d.bind('deckslideActionsInit', function(event) {
...
/**
 * deck.slideActions
 * allows you to set javascript code to be run when a slide becomes current
 * 
 * Example of use:
 *   var action = {
 ...
 *   });
 *
 * After adding all actions, be sure to $.deck('forceRefresh');
 * (This is needed because can load presentation with e.g. #slide-22)
 */
(function($, deck) {
  ...
  var slideActionsInit = function() {    
    ...
    $.deck('extend', 'getSlideIndex', getSlideIndex);
    ...
    $.deck('extend', 'forceRefresh', forceRefresh);
  
    // ------------------------------------------------
  
  
    var slideActions = [];
    var addAction = function(slideIdx, fn) {
      slideActions[slideIdx] = slideActions[slideIdx] || [];
      slideActions[slideIdx].push(fn)
    };
    $[deck]('extend', 'addSlideAction', addAction);
    
    // remove after debug
    $[deck]('extend', 'getSlideActions', function(){
      return slideActions;
    });
    ...
    $d.trigger('deckslideActionsInit')
  
  }  
  
  $d.bind('deck.init', function(event) {
    setTimeout(slideActionsInit, 10);
    $d.unbind(event);
  });
