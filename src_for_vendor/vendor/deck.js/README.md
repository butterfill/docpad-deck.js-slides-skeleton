...
- `deck.hash.js`, `deck.menu.js`, `deck.notes.js`, `deck.transcripts.js`, `deck.remote.js`, `deck.clone.js`, `deck.slide-clone.js`, `deck.velocity.js` - feature extensions described below.
...
## Transcripts (`deck.transcripts.js`)
- Right-side live transcripts panel synced to navigation. Toggle with `t`; `Shift+t` shows an export view; `Alt+t` toggles wide layout.
- Expects a sibling container before `.deck-container`:
  ```html
  <section class="deck-transcripts"><div class="deck-transcripts-container"></div></section>
  ```
  If absent, the plugin auto-inserts it at runtime.
- Resolves a single JSON per deck page by replacing `.html` → `.json` in `window.location.pathname`. Missing JSON is tolerated (fires `deck.transcripts.loadFailed`).
- Inline injection: Elements with class `.transcript` and `id` equal to `transcript_id` are filled from JSON only if empty or matching the sentinel `insert-transcript#<id>-here`. Otherwise existing content is preserved. Missing ids set `[no transcript]` and add `.transcript-missing` (no console warning); emits `deck.transcripts.missingId`.
- Child slide inheritance: If a child slide has no `.transcript`, the panel shows the nearest ancestor (usually the top-level slide) that has one. When the child has its own `.transcript`, it takes precedence.
- API and events: `loadTranscripts`, `getTranscript`, `toggle/show/hideTranscripts`, `toggle/show/hideTranscriptsExport`; events `deck.transcripts.loaded`, `deck.transcripts.loadFailed`, `deck.transcripts.missingId`, `deck.transcripts.renderedForSlide`.
- See `deck.transcripts.spec.md` for full details and LaTeX export mappings.
